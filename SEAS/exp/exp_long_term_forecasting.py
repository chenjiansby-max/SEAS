...
def print_model_macs_and_params(model, input_size):
    if get_model_complexity_info is None:
        print("ptflops is not installed; skip MAC/parameter counting.")
        return
...
class Exp_Long_Term_Forecast(Exp_Basic):
    def __init__(self, args):
        ...
        configs=args
        self.numeric_only = bool(getattr(configs, 'numeric_only', 0))
        ...
        if self.numeric_only:
            ...
            print('Numeric-only mode: skip text encoder and prompt fusion.')
            return
    ...
    def _forward_numeric_backbone(self, batch_x, batch_y, batch_x_mark, batch_y_mark):
        ...
        return outputs[:, -self.args.pred_len:, f_dim:], f_dim
    ...
    def vali(self, vali_data, vali_loader, criterion):
        ...
        with torch.no_grad():
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(vali_loader):
                ...
                if self.numeric_only:
                    outputs, f_dim = self._forward_numeric_backbone(
                        batch_x, batch_y.to(self.device), batch_x_mark, batch_y_mark
                    )
    ...
    def train(self, setting):
        ...
        for epoch in range(self.args.train_epochs):
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark,index) in enumerate(train_loader):
                ...
                batch_y_mark = batch_y_mark.float().to(self.device)
                if self.numeric_only:
                    outputs, f_dim = self._forward_numeric_backbone(batch_x, batch_y, batch_x_mark, batch_y_mark)
