...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        self.text = df_raw[[text_name]][border1:border2].copy()
        self._apply_text_control(text_name)
        # print('text:', self.text)

    def _apply_text_control(self, text_name):
        mode = getattr(self.args, 'text_control_mode', 'aligned')
        if mode == 'aligned':
            return

        fallback = 'No information available'
        values = self.text[text_name].astype(str).tolist()
        seed = int(getattr(self.args, 'text_control_seed', 2024)) + self.set_type * 1009
        rng = np.random.default_rng(seed)

        if mode == 'no_text':
            values = [fallback for _ in values]
        elif mode == 'shuffled_text':
            values = np.array(values, dtype=object)
            rng.shuffle(values)
            values = values.tolist()
        elif mode == 'lagged_text':
            shift = int(getattr(self.args, 'text_shift_steps', 1))
            if shift > 0:
                values = [fallback] * shift + values[:-shift]
            elif shift < 0:
                shift = abs(shift)
                values = values[shift:] + [fallback] * shift
        elif mode == 'random_text':
            values = [
                f"Random semantic control sentence {rng.integers(0, 1000000)}."
                for _ in values
            ]
        else:
            raise ValueError(f"Unsupported text_control_mode: {mode}")

        self.text.loc[:, text_name] = values
        print(f"Applied text_control_mode={mode} on {self.data_path} split={self.set_type}")
